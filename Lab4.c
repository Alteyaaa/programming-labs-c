...
void keyboard_input(double arr[2][100], int coord_num) {
    double x, y;
...
void random_input(double arr[2][100], int coord_num) {
    double min, max;
    printf("Input range boundaries:\n");
    scanf("%lf %lf", &min, &max);
    for (int i = 0; i < coord_num; i++) {
        arr[0][i] = min + (max - min) * ((double) rand() / RAND_MAX);
...
void print_results(double inside[2][100], double outside[2][100], int in_count, int out_count) {
    printf("\nPoints inside the area (%d points):\n", in_count);
    if (in_count > 0) {
        for (int i = 0; i < in_count; i++) {
            printf("Point %d: (%.2f, %.2f)\n", i + 1, inside[0][i], inside[1][i]);
        }
    } else {
...
int main() {
    ...
    for (int i = 0; i < coord_num; i++) {
        double x = points[0][i], y = points[1][i];
        if (check_position_of_point(x, y, R, a)) {
            inside[0][in_count] = x;
            inside[1][in_count] = y;
            in_count++;
        } else {
            outside[0][out_count] = x;
